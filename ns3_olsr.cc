 ...
 * ns3_olsr.cc
 * -----------
 * FANET OLSR Baseline Senaryosu
 *
 * ns3_aodv.cc ile birebir aynı topoloji, mobilite, trafik ve enerji
 * parametreleri — sadece yönlendirme protokolü OLSR.
 * Bu sayede iki protokol arasındaki fark izole şekilde ölçülür.
 *
 * Özellikler:
 *   - 10 UAV + 5 UGV, RandomWaypoint mobilite
 *   - OLSR proaktif yönlendirme (MPR optimizasyonu aktif)
 *   - UDP/CBR trafik: her UGV'den AP'ye
 *   - Metrik: throughput, gecikme, paket kaybı, enerji
 *   - Sonuçlar CSV'ye yazılır (fanet_olsr_results.csv)
 *
 * Derleme:
 *   cp ns3_olsr.cc <ns3-dizini>/scratch/
 *   cd <ns3-dizini>
 *   ./ns3 run scratch/ns3_olsr
 *
 * Karşılaştırma:
 *   python3 metrik_toplayici.py  (her iki CSV'yi birleştirir)
 */

/*
 * ns3_olsr.cc - Düzeltilmiş FANET OLSR Baseline
 * ns-3.41 uyumlu (Tx trace callback kaldırıldı)
 */
...
NS_LOG_COMPONENT_DEFINE("FanetOlsrBaseline");
...
static const std::string OUTPUT_CSV = "fanet_olsr_results.csv";
...
void WriteCSV(const std::vector<NodeMetrics>& metrics, double sim_time)
{
    ...
    csv.close();
    NS_LOG_UNCOND("Sonuclar yazildi: " << OUTPUT_CSV);
}
...
int main(int argc, char* argv[])
{
    ...
    NS_LOG_UNCOND("=== FANET OLSR Baseline ===");
    ...
    // OLSR warm-up: 15 saniye bekle
    double trafficStart = 15.0;
    for (uint32_t i = 0; i < ugvCount; i++) {
        UdpClientHelper client(apAddr, UDP_PORT);
    ...
    NS_LOG_UNCOND("OLSR warm-up: " << trafficStart << "s");
    ...
    for (const auto& m : results) {
        double loss_pct  = (m.txPackets > 0) ? 100.0 * m.lostPackets / m.txPackets : 0.0;
        double tput_kbps = (m.rxBytes * 8.0) / (effectiveTime * 1000.0);
    ...
    WriteCSV(results, effectiveTime);
    Simulator::Destroy();
